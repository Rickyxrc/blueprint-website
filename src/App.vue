        ...
        <template v-if="showFilter">
          <el-form :model="form" label-width="120px">
            ...
            <el-form-item label="蓝图建筑个数">
              <min-max
                v-model:minNum="form.buildingNum.min"
                v-model:maxNum="form.buildingNum.max"
                :settingMax="100000"
                @change="updateData()"
              />
            </el-form-item>
            <el-form-item label="蓝图用电量">
              <min-max
                v-model:minNum="form.yongdianliang.min"
                v-model:maxNum="form.yongdianliang.max"
                :settingMax="100000000"
                @change="updateData()"
              />
            ...
            <el-form-item label="蓝图发电量">
              <min-max
                v-model:minNum="form.fadianliang.min"
                v-model:maxNum="form.fadianliang.max"
                :settingMax="100000000"
                @change="updateData()"
              />
            ...
            <el-table-column label="蓝图原料" width="180">
              <template #default="scope">
                <div v-if="scope.row.蓝图原料.length < 3">
                  <span v-for="i in scope.row.蓝图原料" v-bind:key="i"
                    >{{ i }}/min&nbsp;</span
                  >
                </div>
            ...
            <el-table-column label="蓝图需求建筑" width="180">
              <template #default="scope">
                <div v-if="scope.row.蓝图需求建筑.length < 3">
                  <span v-for="i in scope.row.蓝图需求建筑" v-bind:key="i"
                    >{{ i }}&nbsp;</span
                  >
                </div>
                <div v-else>内容过长，点击查看</div>
